import { Avatar, Box, Button, Card, CardContent, CardHeader, Dialog, DialogContent, DialogContentText, DialogTitle, Grid, IconButton, Paper, Slide, Typography } from "@mui/material";
// import QrCode from "../qrcode/QrCode";
import FacebookIcon from '@mui/icons-material/Facebook';
import LinkedInIcon from '@mui/icons-material/LinkedIn';
import InstagramIcon from '@mui/icons-material/Instagram';
import YouTubeIcon from '@mui/icons-material/YouTube';
// import { heightLogo } from "../../sizes/Sizes";
import DownloadForOfflineIcon from '@mui/icons-material/DownloadForOffline';
import QrCode2Icon from '@mui/icons-material/QrCode2';
import { useDispatch, useSelector } from "react-redux";
import { useNavigate, useParams } from "react-router-dom";
import React, { useEffect } from "react";
import { heightLogo } from "../sizes/Sizes";
import QrCode from "../dashboard/qrcode/QrCode";
import { TapewaveCardGet } from "../reducer/Slices/TapeWaveCardSlice";
import { Notify } from "../reducer/Slices/Notification";
// import QrCode from "../qrcode/QrCode";

const Transition = React.forwardRef(function Transition(props, ref) {
    return <Slide direction="up" ref={ref} {...props} />;
});

export default function TapewaveCard() {
    const data = useSelector(state => state.TapewaveCardReducer)
    const navigate = useNavigate()
    const [open, setOpen] = React.useState(false);
    function downloadContact() {
        var contact = {
            name: data?.name,
            phone: data?.phone_no,
            email: data?.email
        };
        var vcard = "BEGIN:VCARD\nVERSION:4.0\nFN:" + contact.name + "\nTEL;TYPE=work,voice:" + contact.phone + "\nEMAIL:" + contact.email + "\nEND:VCARD";
        var blob = new Blob([vcard], { type: "text/vcard" });
        var url = URL.createObjectURL(blob);

        const newLink = document.createElement('a');
        newLink.download = contact.name + ".vcf";
        newLink.textContent = contact.name;
        newLink.href = url;

        newLink.click();
    }
    const dispatch = useDispatch()
    const params = useParams()

    useEffect(() => {
        try {
            // eslint-disable-next-line
            dispatch(TapewaveCardGet({ params: params?.id, navigate }, { dispatch }))
        } catch (e) {
            console.error(e)
            // eslint-disable-next-line
            dispatch(Notify({ msg: e.response.data.msg }))
        }
    }, [params?.id, dispatch, navigate])

    const handleClickOpen = () => {
        setOpen(true);
    };

    const handleClose = () => {
        setOpen(false);
    };

    return (
        <>
            <Grid
                container
                justifyContent={'center'}
                alignItems={'center'}
            >
                <div
                    style={{
                        position: 'absolute',
                        top: 50,
                        right: 50,
                        zIndex: 10,
                        backgroundColor: '#333333',
                        borderRadius: 20
                    }}
                >
                    <Button onClick={() => navigate(-1)} variant="contained">
                        Back
                    </Button>
                </div>
                <Grid
                    item
                    xs={12}
                    md={6}
                    lg={3}
                    display={'flex'}
                    justifyContent={'center'}
                    alignItems={'center'}
                    borderRadius={5}
                    elevation={0}
                    component={Paper}
                >
                    <Box
                        border={'5px solid #021D48'}
                        borderRadius={5}
                        elevation={10}
                        height={640}
                        width={332}
                        component={Paper}
                        sx={{
                            backgroundImage: `url("data:image/jpeg;base64,")`
                        }}
                    >
                        <Grid
                            item
                            display={'flex'}
                            justifyContent={'center'}
                        >
                            <Box
                                sx={{ borderBottomRightRadius: 10, borderBottomLeftRadius: 10, backgroundColor: '#021D48' }}
                                width={150}
                                height={11}
                            // 
                            >
                            </Box>
                        </Grid>
                        <Grid
                            container
                            overflow={'auto'}
                            height={640 - 11}
                            sx={{
                                borderRadius: 2,
                            }}
                            justifyContent={'space-around'}
                            position={'relative'}
                        >
                            <Grid
                                item
                                xs={12}
                                mt={12}
                                px={4}
                                py={2}
                                borderRadius={3}
                                sx={{
                                    backgroundColor: '#608FFF',
                                }}
                            >
                                <Grid
                                    container
                                    justifyContent={'space-around'}
                                >
                                    <Grid
                                        item
                                        xs={12}
                                    >
                                        <Card sx={{ maxWidth: 345 }} elevation={0}>
                                            <CardHeader
                                                avatar={
                                                    <Avatar aria-label="recipe">
                                                        <img
                                                            height={heightLogo}
                                                            alt="encrypted"
                                                            src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTEQO2ga7Jsm-619O8lg9wp5S0uZtqppyDakw&usqp=CAU"
                                                        />
                                                    </Avatar>
                                                }
                                                sx={{}}
                                                title={<Typography fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600}>{data?.name ? data?.name : 'name'}</Typography>}
                                                subheader={<Typography variant="body2" color="text.secondary" fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600}>{data?.position ? data?.position : 'position'},&ensp;{data?.company ? data?.company : 'company'}</Typography>}
                                            />
                                        </Card>
                                    </Grid>
                                    <Grid
                                        item
                                        xs={4}
                                        sm={3}
                                        md={2}
                                        // lg={1}
                                        p={2}
                                        display={'flex'}
                                        justifyContent={'center'}
                                        alignItems={'center'}
                                    >
                                        <Avatar
                                            sx={{
                                                backgroundColor: '#4D77FA',
                                                cursor: 'pointer',
                                                p: 0.5
                                            }}
                                            onClick={() => { document.getElementById('facebook').click() }}
                                        >
                                            <a href={`${data?.facebook ? data?.facebook : 'https://facebook.com'}`} target="_blank" rel="noreferrer" id="facebook" style={{ color: 'white' }} >
                                                <FacebookIcon />
                                            </a>
                                        </Avatar>
                                    </Grid>
                                    <Grid
                                        item
                                        xs={4}
                                        sm={3}
                                        md={2}
                                        // lg={1}
                                        p={2}
                                        display={'flex'}
                                        justifyContent={'center'}
                                        alignItems={'center'}
                                    >
                                        <Avatar
                                            sx={{
                                                backgroundColor: '#4D77FA',
                                                cursor: 'pointer',
                                                p: 0.5
                                            }}
                                        >
                                            <a href={`${data?.linkedin ? data?.linkedin : `https://linkedin.com`}`} target="_blank" rel="noreferrer" id="linkedin" style={{ color: 'inherit' }} >
                                                <LinkedInIcon />
                                            </a>
                                        </Avatar>
                                    </Grid>
                                    <Grid
                                        item
                                        xs={4}
                                        sm={3}
                                        md={2}
                                        // lg={1}
                                        p={2}
                                        display={'flex'}
                                        justifyContent={'center'}
                                        alignItems={'center'}
                                    >
                                        <Avatar
                                            sx={{
                                                backgroundColor: '#4D77FA',
                                                cursor: 'pointer',
                                                p: 0.5
                                            }}
                                        >
                                            <a href={`${data?.instagram ? data?.instagram : `https://instagram.com`}`} target="_blank" rel="noreferrer" id="instagtam" style={{ color: 'white' }} >
                                                <InstagramIcon />
                                            </a>
                                        </Avatar>
                                    </Grid>
                                    <Grid
                                        item
                                        xs={4}
                                        sm={3}
                                        md={2}
                                        // lg={1}
                                        p={2}
                                        display={'flex'}
                                        justifyContent={'center'}
                                        alignItems={'center'}
                                    >
                                        <Avatar
                                            sx={{
                                                backgroundColor: '#4D77FA',
                                                cursor: 'pointer',
                                                p: 0.5
                                            }}
                                        >
                                            <a href={`${data?.youtube ? data?.youtube : `https://youtube.com`}`} target="_blank" rel="noreferrer" id="instagtam" style={{ color: 'white' }} >
                                                <YouTubeIcon />
                                            </a>
                                        </Avatar>
                                    </Grid>
                                </Grid>
                            </Grid>
                            <Grid
                                item
                                xs={12}
                                mt={0.5}
                                px={4}
                                py={2}
                                borderRadius={3}
                                sx={{
                                    backgroundColor: '#fff',
                                }}
                            >
                                <Card sx={{ maxWidth: 345 }} elevation={0}>
                                    <CardContent>
                                        <Typography fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600} gutterBottom variant="h5" component="div">
                                            About
                                        </Typography>
                                        <Typography fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600} variant="body2" color="text.secondary">
                                            {data?.description ? data?.description : 'description'}
                                        </Typography>
                                    </CardContent>
                                </Card>
                            </Grid>
                            <Grid
                                item
                                xs={12}
                                mt={0.5}
                                px={4}
                                pt={5}
                                pb={8}
                                borderRadius={3}
                                sx={{
                                    backgroundColor: '#fff',
                                }}
                            >
                                <Card sx={{ maxWidth: 345 }} elevation={0}>
                                    <CardHeader
                                        avatar={
                                            <Avatar aria-label="recipe">
                                                R
                                            </Avatar>
                                        }
                                        sx={{}}
                                        title={<Typography fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600}>Contact</Typography>}
                                        subheader={<Typography fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600}>{data?.phone_no ? data?.phone_no : 'phone number'}</Typography>}
                                    />
                                    <CardContent>
                                        <Typography fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600} variant="body2" color="text.secondary">
                                            {data?.email ? data?.email : 'email'}
                                        </Typography>
                                        <Typography fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600} variant="body2" color="text.secondary">
                                            {data?.address ? data?.address : 'address'}
                                        </Typography>
                                        <Typography fontFamily={data?.style === '' ? 'Roboto' : data?.style} fontWeight={600} variant="body2" color="text.secondary">
                                            {data?.city ? data?.city : 'city'},
                                            {data?.state ? data?.state : 'state'}, zipcode
                                        </Typography>
                                        <Typography
                                            variant="body2" color="text.secondary"
                                            fontFamily={data?.style === '' ? 'Roboto' : data?.style}
                                            fontWeight={600}
                                        >
                                            {data?.country ? data?.country : 'country'}
                                        </Typography>
                                    </CardContent>
                                </Card>
                            </Grid>
                        </Grid>
                        <div
                            style={{
                                position: 'absolute',
                                marginTop: -60,
                                marginLeft: 15,
                                backgroundColor: '#333333',
                                borderRadius: 20
                            }}
                        >
                            <IconButton sx={{ p: '10px' }} aria-label="directions" onClick={handleClickOpen}>
                                <QrCode2Icon sx={{ color: "white" }} />
                            </IconButton>
                        </div>
                        <div
                            style={{
                                position: 'absolute',
                                marginTop: -60,
                                marginLeft: 150,
                                backgroundColor: '#333333',
                                borderRadius: 20
                            }}
                        >
                            <Button onClick={() => downloadContact()} variant="contained" endIcon={<DownloadForOfflineIcon />}>
                                Contact
                            </Button>
                        </div>
                    </Box >
                </Grid>
            </Grid>
            <React.Fragment>
                <Dialog
                    open={open}
                    TransitionComponent={Transition}
                    keepMounted
                    onClose={handleClose}
                    aria-describedby="alert-dialog-slide-description"
                >
                    <DialogTitle sx={{ textAlign: 'center', fontFamily: 'Dosis', fontWeight: 600 }}>{"QR Code"}</DialogTitle>
                    <DialogContent>
                        <DialogContentText id="alert-dialog-slide-description">
                            <QrCode url={data?.url} />
                        </DialogContentText>
                    </DialogContent>
                </Dialog>
            </React.Fragment>
        </>
    )
}